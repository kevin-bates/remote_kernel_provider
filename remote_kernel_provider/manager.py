...
from .lifecycle import RemoteLifecycleManager, LocalLifecycleManager
...
class RemoteKernelManager(KernelManagerABC):
    ...
    def start_kernel(self):
        ...
        self.lifecycle_manager = lifecycle_manager_class(kernel_manager=self, proxy_config=self.lifecycle_info.get('config', {}))
    ...
    def kill(self):
        ...
        if self.lifecycle_manager:
            if isinstance(self.lifecycle_manager, RemoteLifecycleManager):
    ...
    def restart_kernel(self, now=False, **kwargs):
        ...
        if isinstance(self.lifecycle_manager, RemoteLifecycleManager) and now:
        ...
        if isinstance(self.lifecycle_manager, RemoteLifecycleManager):  # for remote kernels...
    ...
    def write_connection_file(self):
        ...
        """
        if (isinstance(self.lifecycle_manager, LocalLifecycleManager) or not self.response_address) \
                and not self.restarting:
            # However, since we *may* want to limit the selected ports, go ahead and get the ports using
            # the lifecycle manager (will be LocalLifecycleManager for default case) since the port selection will
