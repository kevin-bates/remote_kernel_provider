...
class BaseLifecycleManagerABC(with_metaclass(abc.ABCMeta, object)):
    """Lifecycle Manager Abstract Base Class.
    ...
    def __init__(self, kernel_manager, lifecycle_config):
        ...
        # the subclass is a LocalLifecycleManager (or YarnLifecycleManager).  This will happen if EG is restarted and the
    ...
    def launch_process(self, kernel_cmd, **kwargs):
        ...
        self.log.debug("BaseLifecycleManager.launch_process() env: {}".format(kwargs.get('env')))
    ...
    def send_signal(self, signum):
        """Send signal `signum` to lifecycle manager.
        ...
        if self.local_proc:
            ...
            if self.ip and self.pid > 0:
                if BaseLifecycleManagerABC.ip_is_local(self.ip):
    ...
    def kill(self):
        ...
        if i > max_poll_attempts:  # Send -9 signal if process is still alive
            if self.local_proc:
                ...
                self.log.debug("BaseLifecycleManager.kill(): {}".format(result))
            else:
                if self.ip and self.pid > 0:
                    if BaseLifecycleManagerABC.ip_is_local(self.ip):
    ...
    def terminate(self):
        ...
        if self.local_proc:
            ...
            self.log.debug("BaseLifecycleManager.terminate(): {}".format(result))
        else:
            if self.ip and self.pid > 0:
                if BaseLifecycleManagerABC.ip_is_local(self.ip):
...
class LocalLifecycleManager(BaseLifecycleManagerABC):
    ...
    """
    def __init__(self, kernel_manager, lifecycle_config):
        super(LocalLifecycleManager, self).__init__(kernel_manager, lifecycle_config)
        kernel_manager.ip = localinterfaces.LOCALHOST

    def launch_process(self, kernel_cmd, **kwargs):
        super(LocalLifecycleManager, self).launch_process(kernel_cmd, **kwargs)
...
class RemoteLifecycleManager(with_metaclass(abc.ABCMeta, BaseLifecycleManagerABC)):
    """Abstract Base Class implementation associated with remote lifecycle managers."""

    def __init__(self, kernel_manager, lifecycle_config):
        super(RemoteLifecycleManager, self).__init__(kernel_manager, lifecycle_config)
    ...
    def launch_process(self, kernel_cmd, **kwargs):
        ...
        super(RemoteLifecycleManager, self).launch_process(kernel_cmd, **kwargs)
    ...
    def detect_launch_failure(self):
        ...
        from the local node.  Proxies like DistributedLifecycleManager use rsh against a remote
    ...
    def cleanup(self):
        ...
        super(RemoteLifecycleManager, self).cleanup()
            ...
            try:
                ...
                if isinstance(e, OSError):
                    if e.errno == errno.ECONNREFUSED and signum == 0:  # Return False since there's no process.
                        return False
                return super(RemoteLifecycleManager, self).send_signal(signum)
            finally:
                sock.close()
        else:
            return super(RemoteLifecycleManager, self).send_signal(signum)
    ...
    def get_lifecycle_info(self):
        ...
        lifecycle_info = super(RemoteLifecycleManager, self).get_lifecycle_info()
    ...
    def load_lifecycle_info(self, lifecycle_info):
        """Captures the base information necessary for kernel persistence relative to remote processes."""
        super(RemoteLifecycleManager, self).load_lifecycle_info(lifecycle_info)
